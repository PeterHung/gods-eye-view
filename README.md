...
本機網頁上方工具列的地球圖示右側提供圓形語言切換按鈕，顯示「中／EN」表示目前語言，點一下即可切換，主要面板、圖層狀態、導覽及金鑰設定會即時切換並記住選擇。首次使用依瀏覽器語言決定預設值；地名、航班代碼、HUD 遙測及來源資料保留原文。

本機網頁可操作地球、圖層與 Provider Settings。部署版也保留右下角「設定服務」入口。線上反向代理部署需先在主機 `source/` 執行 `node scripts/enable-hosted-settings.mjs https://你的網址/gods-eye/`，完整重啟 Node 後，用產生的管理密碼解鎖，即可填入並儲存 API 金鑰。尚未啟用或純靜態部署只顯示設定說明，不會收集金鑰。已實測基本影像、重設地球、USGS 地震圖層及金鑰設定面板；未驗證需金鑰的語音、船舶或擬真 3D。
